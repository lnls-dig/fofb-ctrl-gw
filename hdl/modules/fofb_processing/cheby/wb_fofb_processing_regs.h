...
#define WB_FOFB_PROCESSING_REGS_LOOP_INTLK_CTL_CLR 0x1UL
...
struct wb_fofb_processing_regs {
  ...
  /* [0xc8]: REG (rw) fofb processing loop interlock sources enable control register */
  uint32_t loop_intlk_src_en_ctl;

  /* [0xcc]: REG (rw) fofb processing loop interlock control register */
  uint32_t loop_intlk_ctl;

  /* [0xd0]: REG (rw) fofb processing loop interlock status register */
